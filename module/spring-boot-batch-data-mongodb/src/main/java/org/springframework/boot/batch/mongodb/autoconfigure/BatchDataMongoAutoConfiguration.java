...
public final class BatchDataMongoAutoConfiguration {
	...
	static class SpringBootBatchMongoConfiguration extends MongoDefaultBatchConfiguration {
		...
		SpringBootBatchMongoConfiguration(MongoDatabaseFactory mongoDatabaseFactory,
			...
			this.transactionManager = determineTransactionManager(batchTransactionManager, mongoDatabaseFactory);
		...
		private static MongoTransactionManager determineTransactionManager(
				ObjectProvider<MongoTransactionManager> transactionManager, MongoDatabaseFactory mongoDatabaseFactory) {
			MongoTransactionManager txManager = transactionManager.getIfAvailable();
			if (txManager != null) {
				return txManager;
			}
