...
Here we can see basic `memory`, `heap`, `class loading`, `processor`, `thread pool`
and `uptime` information provided by `SystemPublicMetrics`. The usage and number
of open connections of each managed data source is also provided by `DataSourcePublicMetrics`.

There are some HTTP metrics as well. In this instance the `root` (``/'') and `/metrics` URLs
have returned `HTTP 200` responses `20` and `3` times respectively. It also appears that the
`root` URL returned `HTTP 401` (unauthorized) `4` times.
...
To override the behaviour of an existing `PublicMetrics` such as `SystemPublicMetrics` or
`DataSourcePublicMetrics` create your own version of that bean.
