...
NOTE: the first table is an auto-generated content from metadata harvested at build
time. For reference, the second table is the manual one that we have built so far.

include::../../../target/generated-resources/configuration-items-table.adoc[]
