...
(so a sub-set of `spring.datasource.*` can still be used with your custom configuration).
...
detect the one to uss based on the ones available on the classpath and it also auto-detect
the JDBC driver based on the url.
...
To run an app with that custom `DataSource`, all that is needed really is the connection
information; pool-specific settings can also be provided.
