...
`DataSource` or using JNDI entry.  Here's the algorithm for choosing how it
is auto-configured

* If a JNDI name is set, we locate the data source with that jndi name
...
To configure the data source using jndi, simply set the `spring.datasource.jndi` property
with the proper JNDI name.

NOTE: If a `JndiTemplate` bean is present in the application, we use it for the lookup. If
none is defined, we assume that the default settings are fine to locate the data source. It
is also possible to define a bean with type `JndiDataSourceLookup` to completely override how
the data source is retrieved using its jndi name.
