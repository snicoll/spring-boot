...
configure an in-memory embedded MongoDB on a random port if
`de.flapdoodle.embed:de.flapdoodle.embed.mongo` is available, configure a `MongoTemplate`,
scan for `@Document` classes and configure Spring Data MongoDB repositories. Regular
`@Component` beans will not be loaded into the ApplicationContext`:
...
In-memory embedded mongod generally work well for tests since it is fast and don't
require any developer installation. If, however, you prefer to run tests against a real
MongoDB server you should exclude the embedded mongodb auto-configuration and set the port
to use as `@DataMongoTest` will use a random port by default:
