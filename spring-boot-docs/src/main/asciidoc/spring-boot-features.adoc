...
`@DataElasticsearchTest` can be used if you want to test Elasticsearch applications. By
default, it will configure a local Elasticsearch (if available), configure an
`ElasticsearchTemplate`, scan for `@Document` classes and configure Spring Data
Elasticsearch repositories. Regular `@Component` beans will not be loaded into the
`ApplicationContext`:
