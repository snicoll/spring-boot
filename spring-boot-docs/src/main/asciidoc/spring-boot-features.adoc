...
overriding complex types via a profile. Please be aware of it when you're implementing a
custom configuration namespace.

Let's take an example: assume a `MyPojo` object with `name` and `description` attributes
...
as defined above. If the `dev` profile is enabled however, the `list` will still contain
one entry with name "my another name" and description `null`. In particular,
this configuration _will not_ add a second `MyPojo` instance to the list and it won't
merge the entities.
