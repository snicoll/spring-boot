...
2. Associate a provider to attach a well-defined semantic to a property so that a tool
...
assistance for the key, you could add the following to
...
No more than one provider can be active for a given property but you can specify several
providers in two main cases:

1. A new provider is defined in a latter release with more options and the previous one is
   kept to ensure backward compatibility.
2. Several providers match the property: one offers the best assistance and should be placed
   first while the others allow to offer a _degraded_ service.

The IDE must use the first provider that matches (the first one in the JSON section, that
is). If no provider for a given property is supported, no special content assistance is
provided.
...
The **generic** provider permits any additional values to be provided. Regular value
