...
public class EndpointWebMvcHypermediaManagementContextConfiguration {
	...
	// * could not be enhanced (e.g. "/env/{name}") because their values are "primitive"
	// are
	// * ignored.
	// */
	...
	//
	// private final Map<MediaType, HttpMessageConverter<?>> converterCache = new
	// ConcurrentHashMap<>();
	//
	...
	// Class<? extends HttpMessageConverter<?>> selectedConverterType,
	// MediaType mediaType) {
	// HttpMessageConverter<Object> cached = (HttpMessageConverter<Object>)
	// this.converterCache
	// .get(mediaType);
	// if (cached != null) {
	// return cached;
