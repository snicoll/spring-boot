...
When using the MongoDB store, you can enable initialization of the Spring Batch job repository schema (collections and indexes) and customize the script location:
  ...
  batch:
    data:
      mongodb:
        schema:
          initialize: true
          location: org/springframework/batch/core/schema-mongodb.jsonl
----
