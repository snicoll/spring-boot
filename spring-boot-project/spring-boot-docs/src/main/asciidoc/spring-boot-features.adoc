...
You can use `@DataCouchbaseTest` to test Couchbase applications (without N1QL). By
default, it scans for `@Document` classes and configures Spring Data Couchbase
repositories. Regular `@Component` beans are not loaded into the `ApplicationContext`.

The following example shows the `@DataCouchbaseTest` annotation in use:
