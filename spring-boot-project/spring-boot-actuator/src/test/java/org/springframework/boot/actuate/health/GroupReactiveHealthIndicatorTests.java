...
class GroupReactiveHealthIndicatorTests {
	...
	void groupHealthInvokesIndicatorsByName() {
		...
		StepVerifier.create(createGroupReactiveHealthIndicator("one", "two").health(aggregatedHealth))
				.consumeNextWith((h) -> assertThat(h.getDetails()).containsOnlyKeys("one", "two")).verifyComplete();
	...
	void groupHealthSkipDisabledIndicator() {
		...
		given(aggregatedHealth.health("three")).willReturn(Mono.just(Health.up().build()));
		StepVerifier.create(createGroupReactiveHealthIndicator("one", "two").health(aggregatedHealth))
				.consumeNextWith((h) -> assertThat(h.getDetails()).containsOnlyKeys("one")).verifyComplete();
		verify(aggregatedHealth).health("one");
		verify(aggregatedHealth).health("two");
		verifyZeroInteractions(aggregatedHealth);
	}

	private GroupReactiveHealthIndicator createGroupReactiveHealthIndicator(String... indicatorNames) {
