...
public class QuartzEndpointWebExtension {
	...
	@ReadOperation
	public WebEndpointResponse<Map<String, Object>> quartzJobsOrTriggers(@Selector String jobsOrTriggers)
	...
	private <T> WebEndpointResponse<T> handle(String jobsOrTriggers, ResponseSupplier<T> jobAction,
		...
		if ("jobs".equals(jobsOrTriggers)) {
			return new WebEndpointResponse<>(jobAction.get());
		}
		if ("triggers".equals(jobsOrTriggers)) {
			return new WebEndpointResponse<>(triggerAction.get());
		}
