 ...
 * @since 3.0
 */
public abstract class SpringApplicationHooks {
	 ...
	 */
	public static <T> T withHook(Hook hook, Action<T> action) throws Exception {
		hooks.get().add(hook);
		try {
	 ...
	 * @param action the action to run
	 */
	public static void withHook(Hook hook, Runnable action) {
		hooks.get().add(hook);
		try {
			action.run();
		}
