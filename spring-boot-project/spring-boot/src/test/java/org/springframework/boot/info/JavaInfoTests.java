...
class JavaInfoTests {
	...
	void javaInfoIsAvailable() {
		...
		assertThat(javaInfo.getRuntime()).isNotNull();
		assertThat(javaInfo.getJvm()).isNotNull();
	}

	@Test
	void javaRuntimeEnvironmentInfoIsAvailable() {
		JavaInfo.JavaRuntimeEnvironmentInfo jreInfo = new JavaInfo().getRuntime();
	...
	@Test
	void javaVirtualMachineInfoIsAvailable() {
		JavaInfo.JavaVirtualMachineInfo jvmInfo = new JavaInfo().getJvm();
