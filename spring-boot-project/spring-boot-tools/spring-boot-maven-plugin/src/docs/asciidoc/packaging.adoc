...
An implicit layer resolver is used by default that creates the following layers:
...
[source,xml,indent=0,subs="verbatim,attributes"]
----
	<layers-configuration xmlns="http://www.springframework.org/schema/layers"
						  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
						  xsi:schemaLocation="http://www.springframework.org/schema/layers
...
Each `layer-content` element defines a strategy to include a resource in the layer.
When a resource matches a strategy, it is included in the layer and further strategies are not invoked.
This is illustrated by the `dependencies` and `application` layers that have a "catch-all" include filter used to add any libraries or classes that was not processed by previous strategies.
...
The content of a classes layer can be customized using filters on the name of the file or its location.
