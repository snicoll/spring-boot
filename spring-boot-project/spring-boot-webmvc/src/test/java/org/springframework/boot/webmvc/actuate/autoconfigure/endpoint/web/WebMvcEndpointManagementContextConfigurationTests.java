...
class WebMvcEndpointManagementContextConfigurationTests {
	...
	void contextShouldContainServletEndpointRegistrar() {
		this.contextRunner.withConfiguration(AutoConfigurations.of(WebMvcEndpointManagementContextConfiguration.class))
			.run((context) -> {
	...
	void contextWhenNotServletBasedShouldNotContainServletEndpointRegistrar() {
		new ApplicationContextRunner().withUserConfiguration(InfrastructureConfiguration.class)
