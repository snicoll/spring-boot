...
/**
 * Tests for {@link AutoConfigurationDeferredImportSelector}.
 *
 * @author Stephane Nicoll
 */
public class AutoConfigurationDeferredImportSelectorTests {
	...
	public void multipleSelectorShouldMergeAndSortCorrectly() {
		this.contextRunner.withUserConfiguration(Config.class, AnotherConfig.class)
				.run((context) -> {
					assertThat(getRegisteredAutoConfigurations(context)).containsExactly(
							ConfigA.class.getName(), ConfigB.class.getName(),
							ConfigC.class.getName(), ConfigD.class.getName());
	...
	private List<String> getRegisteredAutoConfigurations(
			AssertableApplicationContext context) {
		return context.getBean(AutoConfigurationImportSelector.REGISTRAR_BEAN_NAME,
	...
	private List<String> getImportedConfigBeans(AssertableApplicationContext context) {
		String shortName = ClassUtils.getShortName(
				AutoConfigurationDeferredImportSelectorTests.class);
