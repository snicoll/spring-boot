...
class WebResourcesRuntimeHintsRegistrarTests {
	...
	@Test
	void registerHintsWithCurrentClasspath() {
		RuntimeHints hints = register(null);
		assertThat(hints.resources().resourcePatterns()).singleElement().satisfies(include("public/*"));
	}

	@Test
	void registerHintsWithHiddenPublicLocation() {
		RuntimeHints hints = register(new FilteredClassLoader(new ClassPathResource("public/")));
