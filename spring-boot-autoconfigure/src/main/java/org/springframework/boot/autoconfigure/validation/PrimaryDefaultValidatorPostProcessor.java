 ...
 * only checking for the presence of the absence {@link javax.validation.Validator}, we
 * should flag the auto-configured validator as primary only if no
 * Spring's {@link Validator} is flagged as primary.
 *
...
class PrimaryDefaultValidatorPostProcessor
	...
	public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata,
		...
		BeanDefinition def = registry.getBeanDefinition("defaultValidator");
		if (def != null
				&& this.beanFactory.isTypeMatch("defaultValidator", LocalValidatorFactoryBean.class)
				&& def.getRole() == BeanDefinition.ROLE_INFRASTRUCTURE) {
			def.setPrimary(!hasPrimaryValidator(registry));
		}
	}

	private boolean hasPrimaryValidator(BeanDefinitionRegistry registry) {
