...
[[packaging.examples.custom-layers-classpath-configuration]]
=== Custom Layers Configuration from the Classpath

When a custom layers configuration is shared across multiple projects, it can be packaged in a separate jar and loaded from the plugin's classpath.
The configuration file must be placed at `META-INF/spring/layers/<name>.xml` in that jar.
The following example loads a configuration named `custom` from a `com.example:layers-configuration` dependency:
...
With this setup, `META-INF/spring/layers/custom.xml` is loaded from the `com.example:layers-configuration` jar on the plugin's classpath.
