...
public class RepackagingTests {
	...
	public void repackagingEnabled() throws IOException {
		...
		// Devtools not excluded by default
		JarFile jarFile = new JarFile(repackageFile);
		assertThat(jarFile.getEntry("lib/spring-boot-devtools-"+BOOT_VERSION+".jar"), notNullValue());
		jarFile.close();
	...
	public void repackagingEnabledExcludeDevtools() throws IOException {
		...
		JarFile jarFile = new JarFile(repackageFile);
		assertThat(jarFile.getEntry("lib/spring-boot-devtools-"+BOOT_VERSION+".jar"), nullValue());
		jarFile.close();
	}
