...
public class InitCommandOptionHandler extends OptionHandler {
	...
	protected void options() {
		...
		this.type = option(Arrays.asList("type", "t"), "The project type to use. Can define the build system to use " +
				"and the structure of the generated project. Check the capabilities of the service (--list) for " +
	...
	private String determineServiceUrl(OptionSet options) {
		return options.has(this.target) ?
				options.valueOf(this.target) : ProjectGenerationRequest.DEFAULT_SERVICE_URL;
	}
