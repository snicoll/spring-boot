...
public class EndpointDiscovererTests {
	...
	@Test
	public void discovertWorksWhenThereAreNoEndpoints() {
		load(EmptyConfiguration.class, (context) -> {
			assertThat(new EndpointDiscoverer(context).discoverEndpoints()).isEmpty();
		});
	}
	...
	public void endpointIsDiscovered() {
		load(TestEndpointConfiguration.class, (context) -> {
			Collection<EndpointInfo<EndpointOperationInfo>> endpoints = new EndpointDiscoverer(
					context).discoverEndpoints();
			assertThat(endpoints).hasSize(1);
			EndpointInfo<EndpointOperationInfo> endpoint = endpoints.iterator().next();
			assertThat(endpoint.getId()).isEqualTo("test");
			Collection<EndpointOperationInfo> operations = endpoint.getOperations();
	...
	public void discoveryFailsWhenTwoEndpointsHaveTheSameId() {
		load(ClashingEndpointConfiguration.class, (context) -> {
			...
			new EndpointDiscoverer(context).discoverEndpoints();
		});
	}
