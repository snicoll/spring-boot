...
public class WebEndpointDiscovererTests {
	...
	public void standardEndpointIsDiscovered() {
		load(TestWebEndpointConfiguration.class, (discoverer) -> {
			Collection<EndpointInfo<WebEndpointOperationInfo>> endpoints = discoverer
					.discoverEndpoints();
			assertThat(endpoints).hasSize(1);
			EndpointInfo<WebEndpointOperationInfo> endpoint = endpoints.iterator().next();
			assertThat(endpoint.getId()).isEqualTo("test");
			Collection<WebEndpointOperationInfo> operations = endpoint.getOperations();
			assertThat(operations).hasSize(3);
			Map<Method, EndpointOperationInfo> operationByMethod = new HashMap<>();
			operations.forEach((operation) -> {
				operationByMethod.put(operation.getOperationMethod(), operation);
			});
			assertThat(operationByMethod).containsKeys(
					ReflectionUtils.findMethod(TestWebEndpoint.class, "getAll"));
		});
	}
	...
	public void webOnlyEndpointIsDiscovered() {
		load(TestWebEndpointConfiguration.class, (discoverer) -> {
			...
			assertThat(operationByMethod).containsKeys(
					ReflectionUtils.findMethod(TestWebEndpoint.class, "getAll"),
					ReflectionUtils.findMethod(TestWebEndpoint.class, "getOne",
							String.class),
					ReflectionUtils.findMethod(TestWebEndpoint.class, "update",
	...
	public void oneOperationIsDiscoveredWhenWebEndpointOverridesOperation() {
		load(OverriddenOperationWebEndpointConfiguration.class, (discoverer) -> {
			...
			assertThat(operationByMethod).containsKeys(ReflectionUtils
					.findMethod(OverriddenOperationWebEndpoint.class, "getAll"));
		});
	}
	...
	public void twoOperationsAreDiscoveredWhenWebEndpointAddsOperation() {
		load(AdditionalOperationWebEndpointConfiguration.class, (discoverer) -> {
			...
			assertThat(operationByMethod).containsKeys(ReflectionUtils.findMethod(
					AdditionalOperationWebEndpoint.class, "getOne", String.class));
			assertThat(operationByMethod).containsKeys(
	...
	private void load(Class<?> configuration, Consumer<WebEndpointDiscoverer> consumer) {
		...
		try {
			consumer.accept(new WebEndpointDiscoverer(new EndpointDiscoverer(context),
					context, Arrays.asList("application/json"),
