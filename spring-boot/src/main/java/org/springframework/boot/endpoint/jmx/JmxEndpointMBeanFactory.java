...
public class JmxEndpointMBeanFactory {
	...
	public Collection<EndpointDynamicMBean> createMBeans(
			Collection<EndpointInfo<EndpointOperationInfo>> endpoints) {
		return endpoints.stream().map((endpointInfo) -> {
			...
			return new EndpointDynamicMBean(endpointInfo.getId(), this.beanFactory,
					this.operationResponseConverter, endpointMBeanInfo);
